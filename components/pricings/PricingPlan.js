      ...
      <div className={`pricing-container container`}>
        ...
        <div className='pricing-wrapper '>
          {data?.PricingCard?.map((Item, index) => {
            return (
              <div
                className={`pricing-table ${Item?.Featured ? 'featured-table' : ''}`}
                key={index}
              >
                ...
                <div className='pricing-innertable'>
                  {Item?.Title && (
                    <div className='pricing-table-title'>
                      <h3 style={Item?.Featured ? { color: '#fff' } : {}}>{Item?.Title}</h3>
                    </div>
                  )}
                  {Item?.SubTitle && (
                    <div className='pricing-table-subtitle'>
                      <p style={Item?.Featured ? { color: '#fff' } : {}}>{Item?.SubTitle}</p>
                    </div>
                  )}
                    ...
                    return (
                      <div className='pricing-table-feature' key={index}>
                        ...
                        <div className='position-relative'>
                          <p
                            className={`${feature?.IsHover ? 'featured-wrapper' : ''}`}
                          ...
                          >
                            ...
                            {feature?.InfoText && feature?.IsHover && (
                              <span className='featured-icon'>
                                {feature?.InfoText && (
                                  <span
                                    className={`${feature?.InfoText ? 'featured-text' : ''} `}
                                  >
                                    {feature?.InfoText}
                                  </span>
                                )}
                  ...
                  >
                    {Item?.Button?.map((button, index) => (
                      <Button key={button?._uid || `button-${index}`} blok={button} Index={index} />
                    ))}
                  </div>
                </div>
              </div>
